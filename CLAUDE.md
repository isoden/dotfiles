...
./manual-tools/sync.sh     # zsh-abbr・ax を Homebrew 非依存でインストール/更新
...
**Homebrew パッケージ管理は mise bootstrap に統合。** `mise.toml`（リポジトリルート、コミット対象）の `[bootstrap.packages]`・`[bootstrap.brew.taps]` が全PC共通パッケージを宣言し、`mise.local.toml`（リポジトリルート、git 管理外）が PC 固有パッケージを宣言する。両ファイルはキー単位でマージされるので、旧 `Brewfile` + `Brewfile.local` と同じ「会社PC/個人PC固有ツールを分離しつつ共通部分を共有する」運用ができる。`mise` の `brew:`/`brew-cask:` パッケージマネージャーは **実物の Homebrew CLI に委譲するのではなく mise 独自実装**（Homebrew 不要で動く）である点に注意。この独自実装には既知の制約がある: `mise bootstrap packages prune` はカスタム tap（`yusukebe/tap` 等）の formula に対して `api/formula/<name>.json` 形式のメタデータを要求し、tap がそれを公開していないと `404` で失敗する（このリポジトリでは `yusukebe/tap/ax` が該当し、`prune` は現状使えない）。`import`（`brew leaves` 相当を `[bootstrap.packages]` へ書き出す）は動作するが、cask は対象外で手動転記が必要。宣言外パッケージを確認する `brew bundle cleanup --dry-run` のような Homebrew 自身のコマンドは、2026-07-24 に brew 本体（`/opt/homebrew/bin/brew`・`Library/Homebrew`）をアンインストール済みのため使えない。`mise bootstrap packages status` の一覧と `mise.toml`/`mise.local.toml` の宣言を目視で突き合わせるか、`/opt/homebrew/Cellar` 配下を直接確認する。

**zsh-abbr・ax は Homebrew に依存せず `manual-tools/sync.sh` で管理する。** 元々 `[bootstrap.packages]` は `brew:olets/tap/zsh-abbr`・`brew:yusukebe/tap/ax` を宣言していたが、mise の `brew:` backend はカスタム tap の formula 取得に `api/formula/<name>.json` という静的 JSON API を常に要求し、`olets/tap`・`yusukebe/tap` はどちらもこれを公開していないため新規 install 時に必ず 404 で失敗する（2026-07-23 実機確認。当初 `prune` 限定の制約と誤認していたが、新規マシンでの初回 install 全般に影響することが判明した）。後継の `github:` backend（GitHub release を直接取得）も検証したが、`ax` は単体バイナリで動く一方、`zsh-abbr` は `zsh-job-queue` という git submodule に依存しており、GitHub の release tarball には submodule の中身が含まれない（GitHub 自体の仕様であり mise 固有の不具合ではないことを `gh release download` で確認済み）ため `zsh-abbr.zsh` の初回 source 時の `git submodule update --init --recursive` が `.git` 不在で失敗する。したがって `github:` backend でも動かせない。実機で確認できた唯一の Homebrew 非依存の導入方法は `git clone --recurse-submodules --single-branch --branch <tag> --depth 1` による直接 clone（`ax` は公式インストーラー `curl -fsSL https://ax.yusuke.run/install | sh`、中身は GitHub release からのバイナリ取得 + sha256 検証後のアトミック配置で `AX_VERSION` によるタグ pin に対応）で、これを `manual-tools/sync.sh` に実装した。gh-skills のような汎用 manifest パーサーは作らず1本のスクリプトに2つの専用インストール処理を直接書いている理由は「2件のために薄いラッパーを増やさない」（`mise bootstrap packages use` の項を参照）という判断と同じだが、gh-skills/sync.sh 自体は実体のある同期処理として存続させている前例にも整合する。冪等性判定は zsh-abbr が `git describe --tags --exact-match` とタグ一致 + submodule 非空チェック、ax が `ax --version`（v無し）と pin タグ（v有り、`#` で除去）の比較で行う。
...
**`.config/mise/config.toml` と `mise.toml` の役割分担。** `.config/mise/config.toml` は `~/.config/mise/config.toml` に symlink される、グローバルな `[tools]`（bun/node 等の言語ランタイムのフォールバック）専用の設定。一方 `mise.toml` はリポジトリルートに置くだけで**どこにも symlink しない** — bootstrap タスクが参照する `{{config_root}}` は、グローバル設定として読み込まれた場合 `$HOME` に解決されてしまい、`gh-skills/sync.sh` の相対パス実行（`dir = "{{config_root}}"`）が壊れるため。

**GitHub 配布の Claude Code skills は `gh skill` で同期（vendoring・extension 化はしない）。** 自作 skill（`claude/skills/`）とは別に、GitHub リポジトリで配布されている skills は `gh-skills/manifest.txt`（`owner/repo skill-name commit-sha` を1行1skillで列挙）が唯一のソース。実体をリポジトリにコピーする vendoring 方式は採用していない — 配布元のライセンス確認の手間（`css-core-architecture` を `.gitignore` に置いた理由と同じ問題）と upstream 更新追従の手動化を避けるため、reference + sha pin 方式（Claude Code 本体の plugin marketplace や Vercel skills.sh も同じ思想）を踏襲している。`gh skill` 自体は `gh extension list` に出てこない GitHub CLI 本体組み込みの preview 機能（extension ではない）なので、`gh-skills/sync.sh`/`gh-skills/dump.sh` を gh extension 化する方向は検討したが見送った — マニフェスト（複数リポジトリ横断でどの skill を入れるか宣言する機能）は `gh skill` 本体には無く、そこは自作スクリプトが引き続き必要になるため。

`mise.toml` の `[tasks.bootstrap]` が `gh-skills/sync.sh` を呼び、各行を `gh skill install <repo> <skill> --pin <sha> --agent claude-code --scope user --force` でインストールする。**`<skill>@<sha>` 構文だけで `--pin` を付けないと `pinned: false` のままになり、誰かが `gh skill update --all` を実行すると黙って上書きされる**（2026-07-22 実機確認）。`--pin` を付けると `pinned: true` になり `gh skill update` から自動的にスキップされる。commit sha を明示指定することで、複数 PC 間で同一バージョンが入る状態を保証する（`[bootstrap.packages]` がバージョンを固定しないのとは対照的）。新しい skill を試すときは pin 無しで `gh skill install` した後 `gh-skills/dump.sh` を実行する。`gh skill update --dry-run` で他ツール（`codex`, `github-copilot` 等）向けに別途インストールされた同名 skill の更新も一覧に混ざって出るが、それらはこのリポジトリの管理対象（`--agent claude-code`）ではないので無視してよい。

`gh skill list --json` の `sourceURL` は、Claude Code 公式 plugin marketplace（`~/.claude/settings.json` の `enabledPlugins`。figma 公式 plugin 等）や Cloudflare 系の組み込み skill では空文字列になる。これらは `gh skill install` で再現できないインストール経路なので、`gh-skills/dump.sh` は `sourceURL` が空のエントリを警告付きでスキップし、マニフェストの対象外にする。`version` は pin 済みなら commit sha だがそれ以外は branch/tag 名になるため、dump 側は 40 桁16進数か判定し、そうでなければ `gh api repos/<owner>/<repo>/commits/<ref>` で commit sha に解決してから書き出す。`gh skill uninstall` 相当のコマンドは無いため、`gh-skills/manifest.txt` から行を消しても実体は残る（手動で削除ディレクトリを消す必要がある）。
...
- スクリプト内でパスを自己解決するときは `${0:A:h}`（絶対パス化して親ディレクトリ）を使う。`gh-skills/` 配下は `:h:h` でリポジトリルート。
